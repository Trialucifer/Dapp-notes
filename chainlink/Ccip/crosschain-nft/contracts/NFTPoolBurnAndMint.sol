 ...
 * 
 * 合约文件地址：https://docs.chain.link/ccip/tutorials/send-arbitrary-data
 */
...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    event MessageReceived(
        ...
        RequestData text // The text that was received.
    );
    ...
    function burnAndSendNFT(
        ...
        address receiver) public returns(bytes32) {
            ...
            * 所以当前调用 transferFrom方法的地址是当前合约地址，但是合约地址是没有授权的
            */
            wnft.transferFrom(msg.sender, address(this), tokenId);
            //burn the wnft before send to ccip
            //先把wnft转到当前合约，然后当前合约来烧掉，需要第一个操作的原因是只有holder才有权限执行burn操作
            wnft.burn(tokenId);
            //因为传输数据的时候，没办法使用复杂的类型，所以使用bytes
    ...
    // 接收CCIP发送过来的信息
    // any2EvmMessage 从任何一条链发送到EVM上的信息
    /**
     * 
    ...
    */
        ...
        override
    {
        ...
        RequestData memory rd = abi.decode(any2EvmMessage.data, (RequestData)); // abi-decoding of the sent text
